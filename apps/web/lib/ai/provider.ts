...
export async function generateStructured<T>(input: { actorId: string; operation: string; system: string; user: string; schema: z.ZodType<T>; jsonSchema: Record<string, unknown> }): Promise<{ value: T; model: string }> {
  ...
  try {
    ...
    const body = await response.json() as { choices?: Array<{ message?: { content?: string } }> }; const content = body.choices?.[0]?.message?.content; if (!content) throw new AIOperationError("invalid_output"); const parsed = input.schema.safeParse(JSON.parse(content)); if (!parsed.success) throw new AIOperationError("invalid_output"); log("info", "ai_request", { operation: input.operation, model, durationMs: Date.now() - started, outcome: "success" }); return { value: parsed.data, model };
  } catch (error) { const normalized = error instanceof AIOperationError ? error : error instanceof Error && error.name === "AbortError" ? new AIOperationError("timeout") : new AIOperationError("unknown"); log("warn", "ai_request", { operation: input.operation, model, durationMs: Date.now() - started, outcome: "fallback", category: normalized.category, providerError: safeErrorCategory(error) }); throw normalized; } finally { clearTimeout(timer); }
}
