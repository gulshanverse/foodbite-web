...
export async function removeCartItem(userId: string, listingId: string) {
  const cart = await prisma.cart.findUnique({ where: { userId } });
  if (cart) await prisma.cartItem.deleteMany({ where: { cartId: cart.id, listingId } });
}
...
export async function createOrderFromCart(userId: string) {
  return prisma.$transaction(async (tx) => {
    ...
    const items: Array<{ listingId: string; sellerId: string; listingName: string; unit: ListingUnit; foodType: FoodType; quantity: number; unitPrice: number; originalUnitPrice: number; lineTotal: number }> = [];
    ...
    return { order, pickupCode: secrets.code, qrToken: secrets.token };
  }, { isolationLevel: "Serializable" });
}
...
export async function markPaymentSuccessful(providerOrderId: string, providerPaymentId: string) {
  return prisma.$transaction(async (tx) => {
    ...
    await tx.payment.update({ where: { id: payment.id }, data: { status: "SUCCESS", providerPaymentId, paidAt: now } });
    for (const reservation of payment.order.reservations) await tx.inventory.update({ where: { listingId: reservation.listingId }, data: { reservedQuantity: { decrement: reservation.quantity }, soldQuantity: { increment: reservation.quantity }, version: { increment: 1 } } });
    return order;
  });
}
...
export async function cancelBuyerOrder(userId: string, id: string) {
  return prisma.$transaction(async (tx) => {
    ...
    if (!order || order.status !== "PENDING_PAYMENT") throw new Error("Only unpaid orders can be cancelled here.");
    for (const reservation of order.reservations) if (reservation.status === "CONFIRMED") {
