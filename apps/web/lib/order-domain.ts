...
export async function getOrCreateCart(userId: string) { return prisma.cart.upsert({ where: { userId }, create: { userId }, update: {}, include: { items: { include: { listing: { include: { category: true, images: { orderBy: { sortOrder: "asc" } }, inventory: true, seller: { include: { business: true } } } } } } } }); }
export async function addCartItem(userId: string, listingId: string, quantity: number) {
  ...
  if (!listing?.inventory || quantity > listing.inventory.availableQuantity) throw new Error("Not enough quantity available.");
  const cart = await getOrCreateCart(userId); const existing = cart.items.find((item) => item.listingId === listingId); const nextQuantity = (existing?.quantity ?? 0) + quantity;
...
export async function updateCartItem(userId: string, listingId: string, quantity: number) {
  const cart = await prisma.cart.findUnique({ where: { userId } }); if (!cart) throw new Error("Cart not found.");
...
export async function removeCartItem(userId: string, listingId: string) { const cart = await prisma.cart.findUnique({ where: { userId } }); if (cart) await prisma.cartItem.deleteMany({ where: { cartId: cart.id, listingId } }); }
