...
export default async function NotificationsPage() {
  ...
  return <main className="mx-auto max-w-3xl px-6 py-12"><p className="text-sm font-semibold uppercase tracking-[.18em] text-[var(--accent)]">Your updates</p><h1 className="mt-3 text-4xl font-bold text-[var(--foreground)]">Notifications</h1><p className="mt-3 text-[var(--muted)]">Order and pickup updates from FoodBite. Transactional notifications stay separate from marketing preferences.</p><section aria-label="Notification list" className="mt-8"><NotificationsList initial={notifications.map((notification) => ({ ...notification, createdAt: notification.createdAt.toISOString(), readAt: notification.readAt?.toISOString() ?? null }))} /></section></main>;
}
