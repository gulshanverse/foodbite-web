...
export function ProfileForm({ initialName }: { initialName: string }) {
  ...
  return <form onSubmit={submit} className="mt-8 max-w-md space-y-5"><label className="block text-sm font-medium">Name<input name="name" defaultValue={initialName} required minLength={2} maxLength={80} className="mt-2 min-h-11 w-full rounded-lg border border-[var(--border)] bg-white px-3 focus:outline-none focus:ring-2 focus:ring-[var(--accent)]" /></label>{message && <p role="status" className="text-sm text-[var(--muted)]">{message}</p>}<button disabled={pending} className="min-h-11 rounded-lg bg-[var(--accent)] px-5 py-3 font-semibold text-white disabled:opacity-60">{pending ? "Saving…" : "Save profile"}</button></form>;
}
