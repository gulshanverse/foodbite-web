...
import { ArrowRight, ChevronLeft, ChevronRight, Clock3, HeartHandshake, MapPin, Sparkles, Store, UtensilsCrossed } from "lucide-react";
import { useEffect, useState } from "react";

const slides = [
  {
    eyebrow: "Discover nearby",
    title: "Good food, right when it matters.",
    copy: "Find eligible surplus food from nearby businesses with clear pickup windows and straightforward pricing.",
    icon: MapPin,
    tone: "coral",
  },
  {
    eyebrow: "For businesses",
    title: "Turn surplus into value.",
    copy: "Publish what is available, manage inventory, and keep every transaction tied to a real pickup or delivery workflow.",
    icon: Store,
    tone: "green",
  },
  {
    ...
    copy: "When eligible surplus cannot be sold, verified recovery partners can help move it toward a useful next chapter.",
    icon: HeartHandshake,
    tone: "sand",
  },
] as const;
  ...
  return (
    <main className="home-page">
      <section className="home-hero">
        <div className="home-hero-copy">
          ...
          <div className="home-actions">
            <Link href="/explore" className="home-primary">Find food near me <ArrowRight className="size-4" /></Link>
            <Link href="/seller" className="home-secondary">For food businesses</Link>
          </div>
          <div className="home-trust-row">
            <span><MapPin className="size-4" /> Nearby first</span>
            <span><Clock3 className="size-4" /> Live pickup windows</span>
            <span><HeartHandshake className="size-4" /> Trust by design</span>
          </div>
        </div>
        ...
        <div className="home-story" aria-label="FoodBite product story">
          <div className={`home-story-stage home-story-${slide.tone}`}>
            ...
            <div className="home-story-card">
              ...
              <h2>{slide.title}</h2>
              <p>{slide.copy}</p>
              <div className="home-story-detail">
                <div><small>DESIGNED AROUND</small><strong>Nearby discovery · clear availability · accountable transactions</strong></div>
                <div className="home-story-line" />
                <div><small>GOOD FOOD</small><strong>Less waste, more value, better next chapters.</strong></div>
              </div>
            </div>
            <div className="home-story-controls">
              <button type="button" onClick={() => setActive((active - 1 + slides.length) % slides.length)} aria-label="Previous story"><ChevronLeft className="size-4" /></button>
              <div className="home-story-dots">{slides.map((item, index) => <button key={item.eyebrow} type="button" aria-label={`Show story ${index + 1}`} aria-current={index === active} onClick={() => setActive(index)} />)}</div>
              <button type="button" onClick={() => setActive((active + 1) % slides.length)} aria-label="Next story"><ChevronRight className="size-4" /></button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="home-section home-cta">
        <div><p className="page-kicker">Start where you are</p><h2>Find your next good meal — or give your surplus a better path.</h2></div>
        <div className="home-cta-actions"><Link href="/explore" className="home-primary">Explore food <ArrowRight className="size-4" /></Link><Link href="/signup" className="home-secondary">Create an account</Link></div>
      </section>
    </main>
  );
}
