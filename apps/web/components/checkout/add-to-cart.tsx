...
export function AddToCart({ listingId, available }: { listingId: string; available: number }) {
  ...
  return <div className="mt-8 rounded-2xl border border-[var(--border)] p-4"><div className="flex items-center gap-3"><label htmlFor="quantity" className="text-sm font-semibold text-[var(--foreground)]">Quantity</label><input id="quantity" type="number" min={1} max={available} value={quantity} onChange={(e) => setQuantity(Math.min(available, Math.max(1, Number(e.target.value) || 1)))} className="w-20 rounded-lg border border-[#cdd9d4] px-3 py-2 text-center" /><button onClick={add} disabled={busy || available < 1} className="flex-1 rounded-xl bg-[var(--foreground)] px-4 py-3 font-semibold text-white disabled:opacity-50">{busy ? "Adding…" : "Add to cart"}</button></div>{message && <p className="mt-3 text-sm text-[var(--muted)]">{message}</p>}<Link href="/cart" className="mt-3 inline-block text-sm font-semibold text-[var(--foreground)] underline">View cart</Link></div>;
}
