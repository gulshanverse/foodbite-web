...
## Phase 11 analytics and platform intelligence

Phase 11 provides live, read-only analytics through `apps/web/lib/analytics-domain.ts`. Metrics are calculated from persisted PostgreSQL records at request time; there is no analytics warehouse, queue, manually maintained counter, or AI dependency. The seller endpoint derives ownership from the authenticated seller relationship, while the admin endpoint requires the explicit `ANALYTICS_READ` permission.

All analytics ranges use UTC timestamps and are bounded to a maximum of 90 days. Supported presets are today, seven days, thirty days, current month, previous month, and validated custom ranges. The UI identifies the reporting window and uses accessible tables rather than decorative or fabricated charts. Query failures show an unavailable state and never affect checkout, payments, inventory, orders, fulfillment, or donations.

Metric definitions are centralized in `metricDefinitions`. Completed order value means the sum of `Order.totalAmount` for orders currently marked `COMPLETED`; it is presented as successful order value rather than accounting revenue. Sold quantity is derived from `OrderItem.quantity` for completed orders. Inventory totals come from the seller-owned `Inventory` rows. Donated and collected quantities come from `Donation.quantity` and `Donation.collectedQuantity`. Donation completion rate is completed donations divided by donations eligible for completion (`COLLECTED`, `COMPLETED`, or `FAILED_COLLECTION`) and is `N/A` when the denominator is zero. Pickup and delivery distributions use persisted `Pickup` and `Delivery` statuses. Unsold inventory is never labeled as waste, and the product does not claim waste reduction without a documented methodology.

Seller analytics expose only seller-owned orders, listings, inventory, donation, and fulfillment aggregates. Admin analytics expose aggregate marketplace, account-role, order, payment, donation, pickup, delivery, and listing distributions without buyer contact details, addresses, credentials, pickup secrets, or raw internal records. Seller analytics CSV export is available through the existing export endpoint with `kind=analytics`; it uses the same bounded range and ownership rules. Admin analytics CSV export is available through the permission-protected admin analytics endpoint with `format=csv`.

The current implementation intentionally uses Prisma database-side `groupBy` and `aggregate` operations with existing ownership/status indexes. No speculative indexes or status-history tables were added because current metrics use actual creation timestamps and current persisted status. Historical status-transition analytics, growth rates, conversion, impressions, ratings, waste impact, predictive forecasting, data warehousing, caching, and background aggregation remain deferred until the source semantics exist and scale requires them.

## Phase 12 production infrastructure and observability

Phase 12 keeps FoodBite as a modular monolith on the existing Next.js, Vercel, and managed PostgreSQL architecture. `runtime-config.ts` validates environment shape and applies stricter required-key checks in production. Optional email, AI, maps, and payment integrations do not make development or readiness fail unless their configured mode requires them.

`logger.ts` is the server-side structured logging boundary. It emits JSON diagnostics with timestamp, event, level, environment, component, request ID, duration, and safe operational metadata. Central redaction removes secrets, credentials, tokens, pickup and QR codes, signatures, prompts, responses, contact data, and addresses. Middleware generates or preserves safe request identifiers and returns them through `x-request-id` along with baseline security headers.

`/api/health` is a minimal liveness check. `/api/ready` validates runtime configuration and performs a bounded `SELECT 1` using the existing Prisma singleton. Readiness failures expose only a dependency label and request ID. Internal reservation and notification workers retain their secret-based authentication and bounded/idempotent state transitions while reporting duration and aggregate outcomes. The payment webhook now has the documented HMAC, schema validation, unique-event idempotency, provider-order lookup, safe failure responses, and structured event logging. AI and analytics log bounded operational metadata only.

No external error-tracking or alerting provider is configured by this repository. The logger and operational runbook document the integration points and provider-level responsibilities without fabricating uptime, latency, error-rate, backup, or alert data. Database migrations remain controlled deployment operations; no destructive migration or new infrastructure service was introduced.
